...
class MachineStatusApp2:
    def __init__(self, root):
        ...
        self.rodi_power_name, self.rodi_alarm_name, self.lift_power_name, self.lift_alarm_name = None, None, None, None
        self.rodi_power_status_value, self.rodi_alarm_status_value, self.lift_power_status_value, self.lift_alarm_status_value = 0, 0, 0, 0
        
        #Remote GPIO
        ips = ['169.254.87.237']
        remotes = [PiGPIOFactory(host=ip) for ip in ips]

        buzzer_alarm = [Alarm_Buzzer(5, pin_factory=r) for r in remotes]  # remote buzzer  
        led_lift_power_alarm = [Lift_Power_Alarm_LED(6, pin_factory=r) for r in remotes]  # remote buzzer  
        led_lift_alarm = [Lift_Alarm_LED(13, pin_factory=r) for r in remotes]  # remote buzzer  
        led_rodi_power_alarm = [RODI_Power_Alarm_LED(19, pin_factory=r) for r in remotes]  # remote buzzer  
        led_rodi_alarm = [RODI_Alarm_LED(26, pin_factory=r) for r in remotes]  # remote buzzer  
        
#         factory = PiGPIOFactory(host='localhost')
#         led_alarm_rodi = LED(17, pin_factory=factory)
#         led_alarm_lift = LED(27, pin_factory=factory)
...
#         self.last_rodi_power_on = None
        ...
        self.gpio_circles = {}
        self.create_gpio_circle("Hot RODI : Power ON", 4, 250, 10)
        self.create_gpio_circle("Hot RODI : Alarm-Siren",27, 250,27)
        self.create_gpio_circle("Hot RODI : Alarm-Revolving Light",21, 250, 44)
        ...
        if self.connect():
            ...
            id = self.db.get_last_created_alarm_name('Hot RODI Power Status: NO POWER!',1)
            self.db.alarm_log_update_to_db(id, datetime.now(),'Hot RODI Power Status: NO POWER!',1)
            
            id = self.db.get_last_created_alarm_name('Hot RODI Alarm Status: ALARM ACTIVE!',1)
            self.db.alarm_log_update_to_db(id, datetime.now(),'Hot RODI Alarm Status: ALARM ACTIVE!',1)
            
            id = self.db.get_last_created_alarm_name('LIFTING TANK Power Status: NO POWER!',2)
            self.db.alarm_log_update_to_db(id, datetime.now(),'LIFTING TANK Power Status: NO POWER!',2)
            
            id = self.db.get_last_created_alarm_name('LIFTING TANK Alarm Status: ALARM ACTIVE!',2)
            self.db.alarm_log_update_to_db(id, datetime.now(),'LIFTING TANK Alarm Status: ALARM ACTIVE!',2)
    ...
    def setup_gpio(self, input_pin, temp, last_active_time, machine_id):
        try:
            ...
            if self.input_signal[input_pin].is_active and temp == 0:
                ...
                elif current_time - last_active_time >= 5:
                    # ignore siren signal becoz it alternating
                    if not input_pin == 27 and not input_pin == 26 :
    ...
    def update_machine_status(self):
        ...
        if self.lift_power_on_temp == 0 :
            ...
            if not self.alarm_power_lift:
                self.lift_power_name = 'LIFTING TANK Power Status: NO POWER!'
                self.lift_power_status_value = 1
                self.add_log_entry(f"{self.dt} : LIFTING TANK Power Status: NO POWER!")
                self.alarm_power_lift = True
                # Update Status
                self.db.alarm_status_update_to_db(3,'LIFTING TANK Power Status: NO POWER!',self.lift_power_status_value ,2)
                # Add Log for No Power Status
                self.db.alarm_log_insert_to_db(datetime.now(),'LIFTING TANK Power Status: NO POWER!',2)
                # Update Log for Power On Status
                id = self.db.get_last_created_alarm_name('LIFTING TANK Power Status: POWER ON',2)
                self.db.alarm_log_update_to_db(id, datetime.now(),'LIFTING TANK Power Status: POWER ON',2)
                
                
        elif self.lift_power_on_temp == 1 :
            if self.alarm_power_lift:
                self.lift_power_name = 'LIFTING TANK Power Status: POWER ON'
                self.lift_power_status_value = 0
                ...
                self.alarm_power_lift = False
                # Update Status
                self.db.alarm_status_update_to_db(3,'LIFTING TANK Power Status: POWER ON',self.lift_power_status_value ,2)
                # Update Log for No Power Status
                id = self.db.get_last_created_alarm_name('LIFTING TANK Power Status: NO POWER!',2)
                self.db.alarm_log_update_to_db(id, datetime.now(),'LIFTING TANK Power Status: NO POWER!',2)
                # Add Log for Power On Status
                self.db.alarm_log_insert_to_db(datetime.now(),'LIFTING TANK Power Status: POWER ON',2)
                
        #Lifting Tank Alarm Status
        # if (self.lift_siren_temp == 1 or self.lift_revolving_light_temp == 1) and self.alarm_lift  :
        if self.lift_revolving_light_temp == 1 and self.alarm_lift  :
            self.lift_alarm_indicator.itemconfig(self.lift_alarm_circle, fill=RED_ON  ,outline=RED_ON )
            self.lift_alarm_name = 'LIFTING TANK Alarm Status: ALARM ACTIVE!'
            self.lift_alarm_status_value = 1
            self.add_log_entry(f"{self.dt} : LIFTING TANK Alarm Status: ALARM ACTIVE!")
            self.alarm_lift = False
            # Update Status
            self.db.alarm_status_update_to_db(4,'LIFTING TANK Alarm Status: ALARM ACTIVE!',self.lift_alarm_status_value ,2)
            # Add Log for Alarm Status
            self.db.alarm_log_insert_to_db(datetime.now(),'LIFTING TANK Alarm Status: ALARM ACTIVE!',2)
            # Update Log for No Alarm Status
            id = self.db.get_last_created_alarm_name('LIFTING TANK Alarm Status: NO ALARM',2)
            self.db.alarm_log_update_to_db(id, datetime.now(),'LIFTING TANK Alarm Status: NO ALARM',2)
            
        elif (self.lift_siren_temp == 0 and self.lift_revolving_light_temp == 0) and not self.alarm_lift:
            self.lift_alarm_indicator.itemconfig(self.lift_alarm_circle, fill=GREY_OFF,outline=GREY_OFF)
            self.lift_alarm_name = 'LIFTING TANK Alarm Status: NO ALARM'
            self.lift_alarm_status_value = 0
            self.add_log_entry(f"{self.dt} : LIFTING TANK Alarm Status: ALARM CLEARED!")
            self.alarm_lift = True
            # Update Status
            self.db.alarm_status_update_to_db(4,'LIFTING TANK Alarm Status: NO ALARM',self.lift_alarm_status_value ,2)
            # Update Log for Alarm Status
            id = self.db.get_last_created_alarm_name('LIFTING TANK Alarm Status: ALARM ACTIVE!',2)
            self.db.alarm_log_update_to_db(id, datetime.now(),'LIFTING TANK Alarm Status: ALARM ACTIVE!',2)
            # Add Log for No Alarm Status
            self.db.alarm_log_insert_to_db(datetime.now(),'LIFTING TANK Alarm Status: NO ALARM',2)
        ...
        if self.rodi_power_on_temp == 0 :        
            if not self.alarm_power_rodi:
                ...
                self.rodi_power_name = 'Hot RODI Power Status: NO POWER!'
                self.rodi_power_status_value = 1
                self.add_log_entry(f"{self.dt} : Hot RODI NO POWER!")
                self.alarm_power_rodi = True
                # Update Status
                self.db.alarm_status_update_to_db(1,'Hot RODI Power Status: NO POWER!',self.rodi_power_status_value ,1)
                # Add Log for No Power Status
                self.db.alarm_log_insert_to_db(datetime.now(),'Hot RODI Power Status: NO POWER!',1)
                # Update Log for Power On Status
                id = self.db.get_last_created_alarm_name('Hot RODI Power Status: POWER ON',1)
                self.db.alarm_log_update_to_db(id, datetime.now(),'Hot RODI Power Status: POWER ON',1)
            ...
            if self.alarm_power_rodi:
                ...
                self.rodi_power_name = 'Hot RODI Power Status: POWER ON'
                self.rodi_power_status_value = 0
                self.add_log_entry(f"{self.dt} : Hot RODI Power Status: POWER RESTORED!")           
                self.alarm_power_rodi = False
                # Update Status
                self.db.alarm_status_update_to_db(1,'Hot RODI Power Status: POWER ON',self.rodi_power_status_value, 1)
                # Update Log for No Power Status
                id = self.db.get_last_created_alarm_name('Hot RODI Power Status: NO POWER!',1)
                self.db.alarm_log_update_to_db(id, datetime.now(),'Hot RODI Power Status: NO POWER!',1)
                # Add Log for Power On Status
                self.db.alarm_log_insert_to_db(datetime.now(),'Hot RODI Power Status: POWER ON',1)

        # Hot RODI Alarm Status
        # if (self.rodi_siren_temp == 1 or self.rodi_revolving_light_temp == 1) and self.alarm_rodi:
        if self.rodi_revolving_light_temp == 1 and self.alarm_rodi:
            self.rodi_alarm_indicator.itemconfig(self.rodi_alarm_circle, fill=RED_ON  ,outline=RED_ON )
            self.rodi_alarm_name ='Hot RODI Alarm Status: ALARM ACTIVE!'
            self.rodi_alarm_status_value = 1
            self.add_log_entry(f"{self.dt} : Hot RODI Alarm Status: ALARM ACTIVE!")
            self.alarm_rodi = False
            # Update Status
            self.db.alarm_status_update_to_db(2,'Hot RODI Alarm Status: ALARM ACTIVE!',self.rodi_alarm_status_value ,1)
            # Update Log for Alarm Status
            self.db.alarm_log_insert_to_db(datetime.now(),'Hot RODI Alarm Status: ALARM ACTIVE!',1)
            # Update Log for No Alarm Status
            id = self.db.get_last_created_alarm_name('Hot RODI Alarm Status: NO ALARM',1)
            self.db.alarm_log_update_to_db(id, datetime.now(),'Hot RODI Alarm Status: NO ALARM',1)
            
        elif (self.rodi_siren_temp == 0 and self.rodi_revolving_light_temp == 0) and not self.alarm_rodi:
            self.rodi_alarm_indicator.itemconfig(self.rodi_alarm_circle, fill=GREY_OFF,outline=GREY_OFF)
            self.rodi_alarm_name = 'Hot RODI Alarm Status: NO ALARM'
            self.rodi_alarm_status_value = 0
            self.add_log_entry(f"{self.dt} : Hot RODI Alarm Status: ALARM CLEARED!")
            self.alarm_rodi = True
            # Update Status
            self.db.alarm_status_update_to_db(2,'Hot RODI Alarm Status: NO ALARM',self.rodi_alarm_status_value ,1)
            # Update Log for Alarm Status
            id = self.db.get_last_created_alarm_name('Hot RODI Alarm Status: ALARM ACTIVE!',1)
            self.db.alarm_log_update_to_db(id, datetime.now(),'Hot RODI Alarm Status: ALARM ACTIVE!',1)
            # Update Log for No Alarm Status
            self.db.alarm_log_insert_to_db(datetime.now(),'Hot RODI Alarm Status: NO ALARM',1)
                ...
                if self.connected:
                    if self.Timer(True, 9):
                        if self.is_connected():
                            ...
                            self.db.status_update_to_db(4, self.rodi_power_on_temp , 1)
                            self.db.status_update_to_db(27, self.rodi_siren_temp , 1)
                            self.db.status_update_to_db(21, self.rodi_revolving_light_temp , 1)
                            self.db.status_update_to_db(13, self.lift_power_on_temp , 2)
                            self.db.status_update_to_db(26, self.lift_siren_temp , 2)
                            self.db.status_update_to_db(23, self.lift_revolving_light_temp , 2)
                            
                            self.db.alarm_status_update_to_db(1, self.rodi_power_name,self.rodi_power_status_value, 1 )
                            self.db.alarm_status_update_to_db(2, self.rodi_alarm_name,self.rodi_alarm_status_value, 1 )
                            self.db.alarm_status_update_to_db(3, self.lift_power_name,self.lift_power_status_value, 2 )
                            self.db.alarm_status_update_to_db(4, self.lift_alarm_name,self.lift_alarm_status_value, 2 )
                        else:
                    ...
                    for buzzer in buzzer_alarm:
                        buzzer.source = (not self.rodi_power_on_temp or not self.lift_power_on_temp or self.rodi_revolving_light_temp or self.lift_revolving_light_temp)
                        
                    for led_lift_power in led_lift_power_alarm:
                        led_lift_power.source = not self.lift_power_on_temp
                    for led_lift in led_lift_alarm:
                        led_lift.source = self.lift_revolving_light_temp
                    for led_rodi_power in led_rodi_power_alarm:
                        led_rodi_power.source = not self.rodi_power_on_temp
                    for led_rodi in led_rodi_alarm:
                        led_rodi.source = self.rodi_revolving_light_temp
