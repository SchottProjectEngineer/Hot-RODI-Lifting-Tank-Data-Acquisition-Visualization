...
#                 query = "INSERT INTO machine_status (start_time, signal, status, machine_id) VALUES (?, ?, ?, ?)"
    ...
    def alarm_log_insert_to_db(self, start_time, alarm_name, machine_id):
        try:
            if self.connection:
                cursor = self.connection.cursor()
                query = "INSERT INTO machine_alarm_log (start_time, alarm_name, machine_id) VALUES (%s, %s, %s)"
#                 query = "INSERT INTO machine_status (start_time, signal, status, machine_id) VALUES (?, ?, ?, ?)"
                cursor.execute(query, (start_time, alarm_name, machine_id))
                self.connection.commit()
            else:
                print("Database Error", "Failed to connect to the database.")
        except pymysql.Error as e:
    ...
    def alarm_log_update_to_db(self, id, stop_time, alarm_name, machine_id):
        try:
            if self.connection:
                cursor = self.connection.cursor()
                query = "UPDATE machine_alarm_log SET stop_time = %s WHERE machine_id = %s AND alarm_name = %s AND id = %s"
                cursor.execute(query, ( stop_time,  machine_id, alarm_name, id))
                self.connection.commit()
            else:
                print("Database Error", "Failed to connect to the database.")
        except pymysql.Error as e:
    ...
    def alarm_status_update_to_db(self,alarm_id, alarm_status, machine_id):
        try:
            if self.connection:
                cursor = self.connection.cursor()
                # query = "INSERT INTO machine_status (stop_time, status, machine_id) VALUES (%s, %s, %s)"
                query = "UPDATE machine_alarm_status SET alarm_status = %s WHERE alarm_id = %s AND machine_id = %s"
                # query = "UPDATE machine_status SET status = 5 WHERE signal_id = 4 AND machine_id = 1"
                # cursor.execute()
                cursor.execute(query, (alarm_status, alarm_id, machine_id))
                # cursor.execute(query)
                self.connection.commit()
            else:
                print("Database Error", "Failed to connect to the database.")
    ...
    def get_last_created_alarm_name(self, alarm_name, machine_id):
        try:
            if self.connection:
                cursor = self.connection.cursor()
                query = "SELECT id FROM machine_alarm_log WHERE alarm_name = %s AND machine_id = %s ORDER BY id DESC LIMIT 1"
                cursor.execute(query, (alarm_name, machine_id,))
                result = cursor.fetchone()
                return result[0] if result else None
            else:
                print("Database Error", "Failed to connect to the database.")
